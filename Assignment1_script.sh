...
cd fastqc_results
unzip '*.zip' #unzip all the files, easier if I am in the directory with all the files
cd ..
...
if [ "$success" -lt "8" ] #threshold. I chose 80%
then
...
join -t$'\t' -e 'NaN' genes.sorted temp_count_file.txt > temp_gene_counts.txt #Joining both files, probablu not necessary, but I want to make sure there are the same number of genes after bedtools coverage
...
do
# 0 hours uninduced
   cat /localdisk/home/data/BPSM/AY21/fastq/100k.fqfiles | grep "$sample" | awk -F '\t' '$4 == "0" {print $1}' > mean_counts/$sample.0hr.replicates.txt #all the names of the replicates of a single sample in zero hours
   ...
   tail -n+2 mean_counts/$sample.0hr.uninduced.samples | awk '{sum=0; for (i=1; i<=NF; i++) {sum=sum+$i;} m=sum/NF; print $0, m; }' | awk '{ print $4 }' > mean_counts/$sample.0hr.uninduced.mean #the mean of each column
   sed -i "1i""$sample"".0hr.uninduced" mean_counts/$sample.0hr.uninduced.mean #and add column title
   rm -fr mean_counts/*samples
#24 hours induced, same as before
...
do
   ...
   awk -v OFS='\t' 'NR!=1 {$3 = ($2 != 0) ? sprintf("%.3f", ($1-$2)/$2) : "NAN"}1' groupwise_comparisons/comparison_columns > groupwise_comparisons/$comp1.vs.$comp2.single_column #Calculating fold change as (y-x)/x
