...
fastqc -t 6 -f fastq $file -o $PWD/fastqc_results
done

#making a file that has both forwards and reverse reads on the same line, so that I can use a single variable for both
touch forward_and_reverse_reads.txt
touch forward_reads.txt
touch reverse_reads.txt
...
cp genes.txt final_counts/gene_counts_1.txt
